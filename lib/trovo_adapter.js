...
var _chnStreamer = process.env.CHANNEL;
var _altOutput = null
...
class TrovoAdapter extends botbuilder.BotAdapter {
    ...
    async _addCookies() {
        this._cookies = [];
        try {
            this._cookies = require(`${require.main.path}/tmp/${process.env.BOTNAME}-cookies.json`);
    ...
    async _saveCookies() {
        ...
        if (saveCookies) {
            fs.mkdir(`${require.main.path}/tmp`, err => {
                if (err) {
                    if (err.code !== 'EEXIST') {
                        throw err;
                    }
                }
                fs.writeFile(`${require.main.path}/tmp/${process.env.BOTNAME}-cookies.json`,
    ...
    async _joinChat() {
        ...
        this.url.pathname = `/chat/${_chnStreamer}`;
        console.info("[TrovoBot]", process.env.BOTNAME, 'is joining', this.url.href);

        if (_altOutput)
            await _altOutput.send("`[TrovoBot] " + process.env.BOTNAME + ' is joining ' + this.url.href + "`");
    ...
    async _isLoggedIn() {
        ...
        try {
            ...
            if (_altOutput)
                await _altOutput.send("`[TrovoBot] " + process.env.BOTNAME + ' is logged in and ready!`');
        } catch (e) {
            if (e instanceof playwright.errors.TimeoutError) {
                let msg = 'The bot may not be logged in correctly.' +
                            'Restart the bot if you have trouble with it responding.' +
                            'You may need to delete the stored cookie in "' + require.main.path + '/tmp/"'
                console.error(msg);
                if (_altOutput)
                    await _altOutput.send(msg);
            } else {
    ...
    async killTrovo() {
        try {
            //await this._browser.close();
            await TrovoAdapter._browser.close();
        } catch (e) {
            console.error('switchWeb:', e);
        }   
    }


    async switchStream(channel) {
        try {
            await this._browser.close();
            _chnStreamer = channel;
            await this._login();
            await this._joinChat();
            await this._isLoggedIn();
            await this._enableNetwork();
    ...
    alternativeOutput(out){
        TrovoAdapter.prototype._altOutput = out;
        TrovoAdapter._altOutput = out;
        //_altOutput = out;
    }
